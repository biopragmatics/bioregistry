...
def main():
    ...
    metaprefix_to_len = {metaprefix: len(getter()) for metaprefix, _, getter in GETTERS}
    ...
    total_cross = sum(x * y for x, y in combinations(metaprefix_to_len.values(), 2))
    print(
        f"There are {intword(total_cross)} possible mappings "
        f"between external registries (Bioregistry v{VERSION})."
    )

    total_cross = sum(min(x) ** 2 for x in combinations(metaprefix_to_len.values(), 2))
    print(
        f"There are {intword(total_cross)} possible one-to-one mappings "
        f"between external registries (Bioregistry v{VERSION})."
    )

    total_bioregistry = sum(min(bioregistry_len, x) ** 2 for x in metaprefix_to_len.values())
    print(
        f"There are {intword(total_bioregistry)} possible "
    ...
    actual_total = sum(
        (count - len(manager.get_registry_invmap(metaprefix)))
        * (bioregistry_len - len(manager.get_registry_map(metaprefix)))
        for metaprefix, count in metaprefix_to_len.items()
    )
    print(
        f"There are {intword(actual_total)} realistic uncurated "
        f"mappings to the Bioregistry (v{VERSION})."
    )
