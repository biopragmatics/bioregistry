...
def main() -> None:
    ...
    df = df[df["prefix"].map(lambda p: bioregistry.get_description(p) is None)] # type:ignore
    df = df[df["prefix"].map(lambda p: bioregistry.get_obofoundry_prefix(p) is None)]# type:ignore
